...
def test_linear_trend_asc():
    ...
    '''
    trend_factor_asc = LinearTrend(descend=False, delta=0.1)
    trend_factor_desc = LinearTrend(descend=True, delta=0.1)
...
def test_linear_trend_big_delta():
    with pytest.raises(Exception) as exc:
        LinearTrend(descend=False, delta=3)
        assert exc
...
def test_linear_trend_delta_amount():
    for delta in np.random.rand(2):
        # print(delta)
        trend_factor_asc = LinearTrend(descend=False, delta=delta)
        trend_factor_desc = LinearTrend(descend=True, delta=delta)
